<template>
    <div class="m-4">
    <RouterLink to="/home" class="text-emerald-600 font-bold"> 🡨 Back to Instrument details</RouterLink>
    ...
    <div class="shadow-sm rounded-sm gap-x-4">
        <div class="grid @max-md:grid-rows-2 md:grid-cols-2">
            ...
            <input type="text" class="input w-7/8 focus:border-green" v-model="assigned"/>
        </div>
        <div class="flex flex-col m-2 md:ml-4">
            <label>Return Date (optional)</label>
            <input type="date" class="input w-7/8 focus:border-green" v-model="return_date"/>
        </div>
        </div>
        <div>
            <button class="btn mx-2 md:ml-4 my-4 align-self-center bg-deep-green text-white" @click="updateAssigned">Assign Instrument</button>
            <button class="btn bg-white border-none hover:shadow-none hover:underline" @click="$router.back()">Cancel</button>
        </div>
    </div>
    </div>
</template>
